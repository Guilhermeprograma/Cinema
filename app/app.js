// Cineplus - Sistema de Reserva de Ingressos
...
const filmes = [
  {
    ...
    cartaz: 'https://www.instagram.com/p/DSdwG2sj3MF/media/?size=1080x1080',
    sessoes: [
  ...
  },
  {
    ...
    cartaz: 'https://source.unsplash.com/featured/300x450?fantasy+movie',
    sessoes: [
  ...
  },
  {
    ...
    cartaz: 'https://source.unsplash.com/featured/300x450?drama+movie',
    sessoes: [
  ...
  },
  {
    ...
    cartaz: 'https://source.unsplash.com/featured/300x450?pilot+movie',
    sessoes: [
...
const iniciar = () => {
  ...
  preencherOfertas();
  atualizarNav();
  mostrarSecao(usuarioLogado ? 'home' : 'login');
};

const mostrarSecao = (id) => {
  document.querySelectorAll('main .pagina').forEach((sec) => sec.classList.add('oculto'));
...
const atualizarNav = () => {
  ...
  if (usuarioLogado) {
    nav.textContent = `Olá, ${usuarioLogado.nome}`;
    nav.onclick = () => {
      logout();
      return false;
    };
...
const listarFilmes = (lista) => {
  const cont = document.getElementById('lista-filmes');
  cont.innerHTML = '';
  lista.forEach((filme) => cont.appendChild(criarCartaoFilme(filme)));
};
...
const renderizarMapaPoltronas = () => {
  ...
  for (let i = 1; i <= 5; i += 1) {
    for (let j = 1; j <= 10; j += 1) {
      const code = `${String.fromCharCode(64 + i)}${j}`;
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'poltrona';
      btn.textContent = code;

      if (sessao.ocupados.includes(code)) {
        btn.classList.add('ocupada');
        btn.disabled = true;
      } else {
        btn.addEventListener('click', () => selecionarPoltrona(code, btn));
      }
...
const confirmarCompra = (event) => {
  ...
  const mensagem = `${nome}, seu ingresso foi confirmado: ${pedido.filme.titulo} às ${pedido.sessao.horario}, poltrona ${pedido.poltrona}, combo ${pedido.combo ? pedido.combo.nome : 'Nenhum'}, total R$ ${preco.toFixed(2)} (pagamento ${pag}).`;
